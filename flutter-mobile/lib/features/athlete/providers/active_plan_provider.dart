// import 'package:flutter_riverpod/flutter_riverpod.dart';
// import 'package:http/http.dart' as http;
// import 'dart:convert';
// import 'package:flutter_secure_storage/flutter_secure_storage.dart';

// final _secureStorage = FlutterSecureStorage();

// /// 🔄 Učitava aktivni plan sa backenda
// final activePlanIdProvider = FutureProvider<int?>((
//   ref,
// ) async {
//   final token = await _secureStorage.read(
//     key: 'accessToken',
//   );
//   if (token == null) throw Exception("No token found");

//   final response = await http.get(
//     Uri.parse('http://192.168.0.49:3001/users/me'),
//     headers: {
//       'Authorization': 'Bearer $token',
//       'Content-Type': 'application/json',
//     },
//   );

//   if (response.statusCode == 200) {
//     final data = jsonDecode(response.body);
//     final activeId = data['activeAssignedPlanId'] as int?;
//     print('✅ Active plan ID: $activeId');
//     return activeId;
//   } else {
//     print('❌ Failed to fetch user info: ${response.body}');
//     throw Exception('Failed to load user info');
//   }
// });

// /// 🔁 Postavlja aktivni plan i resetuje `activePlanIdProvider`
// final setActivePlanProvider = Provider<
//   Future<void> Function(int)
// >((ref) {
//   return (int assignedPlanId) async {
//     final token = await _secureStorage.read(
//       key: 'accessToken',
//     );
//     if (token == null) throw Exception("No token");

//     final response = await http.patch(
//       Uri.parse(
//         'http://192.168.0.49:3001/users/set-active-plan',
//       ),
//       headers: {
//         'Authorization': 'Bearer $token',
//         'Content-Type': 'application/json',
//       },
//       body: jsonEncode({'assignedPlanId': assignedPlanId}),
//     );

//     if (response.statusCode == 200) {
//       print('✅ Active plan set to ID: $assignedPlanId');
//       ref.invalidate(
//         activePlanIdProvider,
//       ); // Reload active plan
//     } else {
//       print(
//         '❌ Failed to set active plan: ${response.body}',
//       );
//       throw Exception('Failed to set active plan');
//     }
//   };
// });
